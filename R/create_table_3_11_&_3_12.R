...
create_t3.11_and_3.12 <- function(data, date_var, data_year, by_var, tablename = NA, topic = NA){
  ...
  output <- output |>
    mutate(adjusted_total =  ifelse({{by_var}} != "unknown",
                                    (total + round_excel(proportion * total_na,0)),0),
           proportion = round_excel(proportion * 100, 1))

  if(quo_name(enquo(by_var)) == "fert_age_grp"){
    output <- output |>
      arrange(match({{by_var}}, fertage_order)) |>
      adorn_totals("row", name = "Grand Total")
  } else {
    output <- output |>
      arrange(match({{by_var}}, dthage_order)) |>
      adorn_totals("row", name = "Grand Total")
  }

...
#' @import janitor
#'
#' @examples t6.5 <- create_t6.5_t6.10(dth_data, sex_value = c("male","female"), age_group = "<5")
#' t6.6 <- create_t6.5_t6.10(dth_data, sex_value = c("male","female"), age_group = "5-14")
#'
create_t6.5_t6.10 <- function(data, sex_value = "male", age_group = "<5"){
  output <- data |>
    filter(doryr == 2022 & sex %in% sex_value & age_grp_lead == age_group) |>
